...
def planarAR(REF_IMAGE_PATH, VIDEO_PATH):
    ...
    while (video.isOpened()):
        ...
        if ret:  ## check whethere the frame is legal, i.e., there still exists a frame
            ...
            corners, ids, rejectedImgPoints = cv2.aruco.detectMarkers(frame,arucoDict, parameters=arucoParameters)
            corners=np.array(corners[0][0]).astype(int)
            # print(corners)
            # print(corners.shape,'corners.shape')
            # print(ref_corns.shape,'ref_corns.shape')
            ...
            # print(corners[:,1])
            # print(np.min(corners[:,1]))
            warping(ref_image, frame, H,np.min(corners[:,1]),np.max(corners[:,1]),np.min(corners[:,0]),np.max(corners[:,0]), direction='b')
