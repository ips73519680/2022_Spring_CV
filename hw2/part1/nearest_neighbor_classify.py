...
def nearest_neighbor_classify(train_image_feats, train_labels, test_image_feats):
    ...
    # print('nearest_neighbor_classify start')

    k = 3
    test_predicts=[]

    train_labels=np.array(train_labels)
    dis=distance.cdist(test_image_feats,train_image_feats,metric='braycurtis')
    for row in dis:
        min= np.argpartition(row,k)[:k]
        test_predicts.append(mode(train_labels[min])[0][0])
